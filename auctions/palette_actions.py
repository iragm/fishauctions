...
NOT_A_SKILL: dict[str, str] = {
    ...
    "ContactView": (
        "Emails the site owner a paragraph somebody wrote in their own words, and its whole purpose "
        "is to work with no account: it is the App Store Support URL, opened by a reviewer with no "
        "session. An assistant reaching it is already signed in, and a signed-in person is shown the "
        "address itself on the FAQ -- so the capability the form provides is one the caller does not "
        "need. request_a_skill is where an agent records something this site could not do. "
        "go_to_page opens the page."
    ),
