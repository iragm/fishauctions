...
def update_email_templates(apps, schema_editor):
    ...
    for template in EmailTemplate.objects.filter(html_content__contains=OLD_THUMBNAIL_SNIPPET):
        ...
        template.save()
        updated += 1
    # Warn about templates that contain lot.thumbnail but not the expected old snippet
    for template in EmailTemplate.objects.filter(html_content__contains="lot.thumbnail").exclude(
        html_content__contains=NEW_THUMBNAIL_SNIPPET
    ):
        skipped.append(template.name)
    if updated:
        logger.info("Updated %d email template(s) for URL-only thumbnail support.", updated)
    if skipped:
        logger.warning(
            "Could not update thumbnail snippet in template(s): %s — snippet may have been manually modified.",
            ", ".join(skipped),
        )
