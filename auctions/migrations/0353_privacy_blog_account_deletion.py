# Generated manually: document account deletion in the privacy blog post
from django.db import migrations


def update_privacy_blog_post(apps, schema_editor):
    # Same shape as 0326: BlogPostView renders body_rendered, and the historical MarkdownField in a
    # data migration doesn't regenerate it on save, so render the HTML here and write both fields.
    # privacy_content below is the full, current text (now including the account-deletion section
    # that /account/delete/ points at).
...
When the deletion goes through, we remove your username, email address, password, any linked Google account, your name, phone number, mailing address, location, preferences, watched lots, search and browsing history, the devices you've used the app on (and their push notification tokens), any PayPal or Square account you connected, and your contact details in any club mailing lists we sync to Mailchimp or Brevo.

Some records stay, because they're other people's records too:

- Auctions you took part in keep your bidder number and the amounts on your invoices, so that sellers' payouts and clubs' past auctions still add up.  Your name and contact details are removed from them.
...
- Club membership records that a club admin created or has edited stay with the club, including dues paid and award points.  We remove the link to your account and mark you as do-not-contact so the club stops emailing you; if you want the club's own record deleted, ask the club.  A membership you signed yourself up for, that no admin has edited, is deleted with your account.
...
class Migration(migrations.Migration):
    dependencies = [
        ("auctions", "0352_clubmember_admin_edited_and_more"),
    ]
