...
class CurrencyCustomizationTests(StandardTestCase):
    ...
    def test_currency_change_invalidates_auction_cache(self):
        """Test that changing currency preference invalidates auction cached_stats"""
        # Set initial currency and create cached stats
        self.user.userdata.preferred_currency = "USD"
        self.user.userdata.save()

        # Create auction with cached stats
        self.online_auction.cached_stats = {"lot_sell_prices": {"labels": ["$1-2"]}}
        self.online_auction.save()

        # Verify cached stats exist
        self.online_auction.refresh_from_db()
        self.assertIsNotNone(self.online_auction.cached_stats)

        # Change currency
        self.user.userdata.preferred_currency = "GBP"
        self.user.userdata.save()

        # Verify cached stats were cleared
        self.online_auction.refresh_from_db()
        self.assertIsNone(self.online_auction.cached_stats)

    def test_currency_change_only_affects_user_auctions(self):
        """Test that changing currency only invalidates auctions created by that user"""
        # Create another user and their auction
        ...
        other_auction.cached_stats = {"lot_sell_prices": {"labels": ["$1-2"]}}
        other_auction.save()

        # Set cached stats for original user's auction
        self.online_auction.cached_stats = {"lot_sell_prices": {"labels": ["$1-2"]}}
        self.online_auction.save()

        # Change original user's currency
        self.user.userdata.preferred_currency = "EUR"
        self.user.userdata.save()

        # Original user's auction cache should be cleared
        self.online_auction.refresh_from_db()
        self.assertIsNone(self.online_auction.cached_stats)

        # Other user's auction cache should remain
        other_auction.refresh_from_db()
        self.assertIsNotNone(other_auction.cached_stats)
