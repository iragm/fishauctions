...
class SkillAuditTests(SimpleTestCase):
    ...
    def test_no_view_is_both_covered_and_excused(self):
        """SKILLS and NOT_A_SKILL are a partition: no view may be in both."""
    ...
    def test_no_excused_view_is_reimplemented_by_a_registered_action(self):
        """A resolver docstring claiming to be an excused view's body means the view is really covered."""
...
class SkillTestCase(StandardTestCase):
    """Runs skills directly; ``run_action`` is the gate, so no model is needed."""
...
class HistoryVocabularyTests(SimpleTestCase):
    """Every ``about`` synonym names a real history category, so a typo fails the build."""
...
class SkillPromptTests(StandardTestCase):
    ...
    def test_every_registered_action_reaches_a_superusers_tool_list(self):
        """Every registered action reaches a superuser's tool list, palette or (for ``mcp_only``) MCP."""
...
class CreateAuctionTests(SkillTestCase):
    ...
    def test_copying_carries_the_fees_and_leaves_the_dates_alone(self):
        ...
        # To the day: date_start is parsed in the user's timezone.
    ...
    def test_with_no_source_named_it_copies_their_most_recent(self):
        ...
        self.assertTrue(result.get("ok"), result)
        # Both fixtures share a date_start, so either is a valid source.
...
class LotImageTests(SkillTestCase):
    ...
    def test_adding_a_picture_from_the_internet(self):
        ...
        # The catch-all isn't printed on the lot page (LotImage.source_display), but is stored.
...
class AuctionWideLotImageTests(SkillTestCase):
    """An auction admin adding pictures to other sellers' lots, including through club permissions."""
...
class ClubManagedParticipantTests(ClubSkillTestCase):
    """update_person in a club-managed auction writes to the ClubMember and says it did."""
    ...
    def test_running_the_auction_is_enough_to_fix_somebody_in_it(self):
        """Administering the auction is enough to fix a participant, without a club role. See
        _update_through_the_club.
        """
...
class UndoCheckInTests(SkillTestCase):
    """One person per call, on purpose."""
...
class CopyingAnAuctionTests(SkillTestCase):
    """``copy_users_when_copying_this_auction`` copies the people, not their check-in state."""
    ...
    def test_a_club_managed_copy_never_takes_the_people(self):
        """A club-managed copy never copies people; check-in mode adds them at the door."""
...
class LotSpeciesTests(SkillTestCase):
    """Fixing a lot's species: name it, add a name, or add the species."""
    ...
    def test_an_admin_teaches_the_site_the_lot_name_and_a_seller_does_not(self):
        """An admin's choice teaches the global cache; a seller's doesn't, same rule as LotAdmin."""
        # Sellers can only edit while submission is open; lot_submission_end_date is derived on save.
...
class PointsDeskTests(ClubSkillTestCase):
    """The breeder award review desk: waiting, denied, and never-ticked lots."""
    ...
    def setUp(self):
        ...
        # Off, so there's a queue to review.
    ...
    def test_my_points_forecasts_the_auction(self):
        ...
        # Only eligible, undecided lots count.
...
class PlaceBidTests(SkillTestCase):
    ...
    def setUp(self):
        super().setUp()
        # Someone else's open online lot, to bid on.
        ...
        # Bidding is held for 20 minutes after date_posted, which is auto_now_add.
    ...
    def test_the_answer_says_it_cannot_be_taken_back(self):
        ...
        # Bids can't be reversed, so no undo is offered.
...
class SendingAMembershipCardTests(ClubSkillTestCase):
    ...
    def test_an_admin_can_send_another_members_card(self):
        ...
        # Sent, not shown. See MembershipCardPrivacyTests.
...
class MembershipCardPrivacyTests(ClubSkillTestCase):
    """A membership number and barcode are a credential: club admins can send a card, never read one."""
    ...
    def _numbers_in(self, result):
        """Every membership number anywhere in a result, found structurally.

        A substring search false-alarms when a UUID in a URL contains another member's number.
        """
        found: list[int] = []
...
class SeveralLotsOfTheSameThingTests(SkillTestCase):
    """``count`` is the number of lots; ``quantity`` is fish per lot."""
...
class PriceHistoryTests(SkillTestCase):
    """Price history covers only auctions this person is part of."""
...
class StartingPriceTests(SkillTestCase):
    ...
    def test_a_lot_with_history_behind_it_gets_a_number(self):
        ...
        # The lower quartile of 4, 6 and 12, rounded down.
...
class PageOnlyWriteRegistryTests(SimpleTestCase):
    """The ``mcp_only`` writes as entries in SKILLS and NOT_A_SKILL."""

    #: view -> skill, for everything moved out of ``NOT_A_SKILL``. Written out so moving one back
    #: is deliberate.
    MOVED = {
    ...
    def test_no_excuse_still_argues_about_speech(self):
        ...
        for view, reason in palette_actions.NOT_A_SKILL.items():
            for phrase in speech:
                # A reason may mention speech, just not rest on it alone.
...
class RemoveBidTests(SkillTestCase):
    ...
    def setUp(self):
        ...
        # A bidder who hasn't joined can't name the lot, so the refusal would pass for the wrong reason.
...
class ClubPageOnlyWriteTests(ClubSkillTestCase):
    ...
    def setUp(self):
        super().setUp()
        # ``_bap_club_or_problem`` refuses clubs not running the program.
        self.club.enable_breeder_award_program = True
        self.club.save()
...
class PermissionSeparationTests(SkillTestCase):
    """Administering a club and administering an auction are separate permissions.

    Two crossovers are deliberate and asserted:

    * club ``permission_admin`` or ``permission_manage_auctions`` administers the club's auctions
      (``Auction.permission_check``);
    * club ``permission_add_edit`` may add and edit people in a club-managed auction
      (``views.user_can_add_edit_people``).

    An auction admin never gains any standing in the club.
    """
    ...
    def setUp(self):
        ...
        # Runs the auction, with no ClubMember row.
        ...
        # Runs the club's member list, but isn't an auction admin.
        ...
        # ``Auction.lots_qs`` joins through ``auctiontos_seller``; without one the lot is invisible.
    ...
    def test_a_membership_secretary_is_not_an_auction_admin(self):
        ...
        # Must be a permission refusal, not "no such lot".
