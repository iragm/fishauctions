...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        invoices = Invoice.objects.filter(email_sent=False, status='UNPAID',auctiontos_user__email__isnull=False)
        for invoice in invoices:
            ...
            if invoice.auction:
                ...
                try:
                    location = invoice.auctiontos_user.pickup_location
                except:
                    pass
            elif invoice.seller:
            ...
            mail.send(
                ...
                context={'subject': subject, 'name': invoice.auctiontos_user.name, 'domain': current_site.domain, 'location': location, "invoice": invoice},
            )
            #self.stdout.write(f'Emailed {invoice.auctiontos_user.name} that their invoice {invoice} is ready for payment')
