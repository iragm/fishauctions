...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        pageviews = PageView.objects.filter(ip_address__isnull=False, latitude=0, longitude=0).order_by('-date_start')[:10000]
        for view in pageviews:
            other_view_with_same_ip = PageView.objects.exclude(latitude=0, longitude=0).filter(ip_address=view.ip_address).order_by('-date_start').first()
                        ...
                        try:
                            if view.ip_address == value['query']:
                                if value['status'] == "success":
                                    ...
                                    print(f"IP {view.ip_address} may not be valid - verify it and set their location manually")
                        except Exception as e:
                            print(e)