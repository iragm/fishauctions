...
urlpatterns = [
    ...
    #
    # Two mounts, on purpose. The toolkit's own URLs live under /o/. The discovery documents are
    # mounted a *second* time at the domain root, because RFC 8414 and RFC 9728 both put them at the
    # origin and that is the first place Claude looks; without it they are only at
    # /o/.well-known/..., a fallback some clients never try, and the symptom is "couldn't reach the
    # MCP server" with no request arriving at all.
    #
    # The separate instance namespace keeps reverse("oauth2_provider:...") pointing at /o/.
