...
- **A self-scheduling task needs a watchdog on the beat.** `update_auction_stats` re-arms itself at
  ...
  `ensure_auction_stats_task_scheduled` is one indexed lookup every 15 minutes that re-arms it.
...
- It iterates **primary keys and re-fetches**, bounded to `BATCH_SIZE`. The old loop materialised
  the whole unchecked queryset and so reached rows an earlier iteration had already deleted.
